  ...
  return (
    <div className="p-6 space-y-6" style={{ backgroundColor: '#12110F', minHeight: '100%' }}>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold mb-1" style={{ color: '#F3ECDD', fontFamily: 'Playfair Display' }}>Gallery</h1>
          <p className="text-sm" style={{ color: '#B8B1A8' }}>{albums.length} albums · {albums.reduce((s, a) => s + a.images, 0)} images</p>
        </div>
      ...
      {activeTab === 'albums' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {albums_.map(album => (
            <div key={album.id} className="rounded-2xl overflow-hidden group" style={{ backgroundColor: '#1B1917', border: '1px solid rgba(201,168,106,0.10)' }}>
              <div className="relative h-40 overflow-hidden">
                ...
                <div className="absolute top-3 right-3 flex gap-1">
                  {!album.visible && (
                    <span className="px-2 py-1 rounded-full text-xs" style={{ backgroundColor: 'rgba(155,45,62,0.8)', color: '#F3ECDD' }}>Hidden</span>
                  )}
                </div>
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="text-sm font-medium" style={{ color: '#F3ECDD' }}>{album.name}</h3>
                    <p className="text-xs mt-1" style={{ color: '#B8B1A8' }}>{album.description}</p>
                  </div>
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => toggleVisible(album.id)} className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ color: '#B8B1A8', backgroundColor: 'rgba(184,177,168,0.1)' }}>
          ...
          {/* Add album */}
          <button className="rounded-2xl border-2 border-dashed flex flex-col items-center justify-center p-8 transition-all h-48" style={{ borderColor: 'rgba(201,168,106,0.2)', color: '#B8B1A8' }}>
        ...
        <div>
          <p className="text-sm mb-4" style={{ color: '#B8B1A8' }}>Drag to reorder images shown in the public gallery</p>
            ...
            {featuredImages.map((img, i) => (
              <div key={i} className="group rounded-xl overflow-hidden relative" style={{ border: '1px solid rgba(201,168,106,0.15)' }}>
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <GripVertical size={24} style={{ color: '#F3ECDD' }} className="cursor-grab" />
                </div>
                ...
                <div className="p-2">
                  <p className="text-xs truncate" style={{ color: '#B8B1A8' }}>{img.caption}</p>
                </div>
              </div>
            ))}
          </div>

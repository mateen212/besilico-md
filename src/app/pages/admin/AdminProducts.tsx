  ...
  return (
    <div className="p-6 space-y-6" style={{ backgroundColor: '#12110F', minHeight: '100%' }}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold mb-1" style={{ color: '#F3ECDD', fontFamily: 'Playfair Display' }}>Products</h1>
          <p className="text-sm" style={{ color: '#B8B1A8' }}>{products.length} items on the menu</p>
        </div>
        <div className="flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm transition-all" style={{ backgroundColor: 'rgba(201,168,106,0.1)', color: '#C9A86A', border: '1px solid rgba(201,168,106,0.2)' }}>
            <Upload size={15} /> Export
          </button>
          <Link to="/admin/products/new" className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm" style={{ backgroundColor: '#C9A86A', color: '#12110F', fontWeight: 500 }}>
      ...
      <div className="rounded-2xl p-4" style={{ backgroundColor: '#1B1917', border: '1px solid rgba(201,168,106,0.10)' }}>
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto">
            {categories.map(c => (
              <button
                ...
                style={{
                  backgroundColor: category === c ? '#C9A86A' : 'rgba(201,168,106,0.08)',
                  color: category === c ? '#12110F' : '#B8B1A8',
          ...
          <button className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm" style={{ backgroundColor: 'rgba(201,168,106,0.08)', color: '#B8B1A8', border: '1px solid rgba(201,168,106,0.12)' }}>
            <Filter size={15} /> Filter
          </button>
        </div>

        {selected.length > 0 && (
          <div className="mt-3 pt-3 flex items-center gap-3" style={{ borderTop: '1px solid rgba(201,168,106,0.10)' }}>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: '#1B1917', border: '1px solid rgba(201,168,106,0.10)' }}>
        <table className="w-full">
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(201,168,106,0.10)' }}>
              ...
              <th className="p-4 text-left text-xs font-medium tracking-wider uppercase" style={{ color: '#B8B1A8' }}>Product</th>
              <th className="p-4 text-left text-xs font-medium tracking-wider uppercase hidden md:table-cell" style={{ color: '#B8B1A8' }}>Category</th>
              <th className="p-4 text-left text-xs font-medium tracking-wider uppercase" style={{ color: '#B8B1A8' }}>Price</th>
              <th className="p-4 text-left text-xs font-medium tracking-wider uppercase hidden lg:table-cell" style={{ color: '#B8B1A8' }}>Orders</th>
              <th className="p-4 text-left text-xs font-medium tracking-wider uppercase hidden lg:table-cell" style={{ color: '#B8B1A8' }}>Featured</th>
              <th className="p-4 text-left text-xs font-medium tracking-wider uppercase" style={{ color: '#B8B1A8' }}>Status</th>
              <th className="p-4 text-left text-xs font-medium tracking-wider uppercase" style={{ color: '#B8B1A8' }}>Available</th>
          ...
          <tbody>
            {filtered.map((product, i) => (
              <tr
                ...
                style={{
                  borderBottom: i < filtered.length - 1 ? '1px solid rgba(201,168,106,0.06)' : 'none',
                  backgroundColor: selected.includes(product.id) ? 'rgba(201,168,106,0.05)' : 'transparent',
                }}
              >
                <td className="p-4">
                ...
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-medium" style={{ color: '#F3ECDD' }}>{product.name}</div>
                    </div>
                  </div>
                </td>
                <td className="p-4 hidden md:table-cell">
                  <span className="text-xs px-2.5 py-1 rounded-full" style={{ backgroundColor: 'rgba(201,168,106,0.1)', color: '#C9A86A' }}>
                    {product.category}
                  </span>
                </td>
                <td className="p-4">
                  <span className="text-sm font-medium" style={{ color: '#F3ECDD' }}>€{product.price}</span>
                </td>
                <td className="p-4 hidden lg:table-cell">
                  <span className="text-sm" style={{ color: '#B8B1A8' }}>{product.orders}</span>
                </td>
                <td className="p-4 hidden lg:table-cell">
        ...
        {/* Pagination */}
        <div className="flex items-center justify-between px-4 py-3" style={{ borderTop: '1px solid rgba(201,168,106,0.10)' }}>
          <span className="text-sm" style={{ color: '#B8B1A8' }}>Showing {filtered.length} of {products.length} products</span>
          <div className="flex items-center gap-2">
            <button className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ color: '#B8B1A8', border: '1px solid rgba(201,168,106,0.15)' }}>
              <ChevronLeft size={15} />
            </button>
            <span className="text-sm px-3 py-1 rounded-lg" style={{ backgroundColor: '#C9A86A', color: '#12110F' }}>1</span>
            <button className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ color: '#B8B1A8', border: '1px solid rgba(201,168,106,0.15)' }}>

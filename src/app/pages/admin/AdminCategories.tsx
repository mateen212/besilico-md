...
const defaultCategories = [
  ...
  { id: 6, name: 'Drinks', slug: 'drinks', products: 10, visible: true, color: '#B8B1A8', description: 'Non-alcoholic and cocktails' },
  ...
  return (
    <div className="p-6 space-y-6" style={{ backgroundColor: '#12110F', minHeight: '100%' }}>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold mb-1" style={{ color: '#F3ECDD', fontFamily: 'Playfair Display' }}>Categories</h1>
          <p className="text-sm" style={{ color: '#B8B1A8' }}>{categories.length} categories · {categories.reduce((s, c) => s + c.products, 0)} total products</p>
        </div>
      ...
      {adding && (
        <div className="rounded-2xl p-5" style={{ backgroundColor: '#1B1917', border: '1px solid rgba(201,168,106,0.2)' }}>
          <h3 className="text-base font-semibold mb-4" style={{ color: '#F3ECDD' }}>New Category</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-xs uppercase tracking-wider mb-2" style={{ color: '#B8B1A8' }}>Name *</label>
            ...
            <div>
              <label className="block text-xs uppercase tracking-wider mb-2" style={{ color: '#B8B1A8' }}>Description</label>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ backgroundColor: '#1B1917', border: '1px solid rgba(201,168,106,0.10)' }}>
        <div className="p-4" style={{ borderBottom: '1px solid rgba(201,168,106,0.10)' }}>
          <p className="text-xs uppercase tracking-wider" style={{ color: '#B8B1A8' }}>Drag to reorder categories</p>
        </div>
            ...
            <div key={cat.id} className="flex items-center gap-4 p-4 transition-all group" style={{ opacity: cat.visible ? 1 : 0.5 }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium" style={{ color: '#F3ECDD' }}>{cat.name}</span>
                ...
                <p className="text-xs mt-0.5" style={{ color: '#B8B1A8' }}>{cat.description}</p>
              </div>
              ...
              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button onClick={() => toggleVisible(cat.id)} className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ color: '#B8B1A8', backgroundColor: 'rgba(184,177,168,0.1)' }}>
